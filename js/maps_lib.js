...
*/
        ...
        $("#cap-slider").slider({
            ...
            values: [0, 100],
            slide: function (event, ui) {
                $("#cap-selected-start").html(commafy(ui.values[0]));
                $("#cap-selected-end").html(commafy(ui.values[1]));
            },
            stop: function(event, ui) {
                self.doSearch();
            }
    ...
    MapsLib.prototype.doSearch = function () {
        ...
        var type_column = "'Application'";
        var tempWhereClause = [];
        if ( $("#cbType1").is(':checked')) tempWhereClause.push("geothermal");
        if ( $("#cbType2").is(':checked')) tempWhereClause.push("biomass");
        if ( $("#cbType3").is(':checked')) tempWhereClause.push("heat recovery");
        if ( $("#cbType4").is(':checked')) tempWhereClause.push("solar");
